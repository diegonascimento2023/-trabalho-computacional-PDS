...
#
# Itens:
#   (a) Implementar o algoritmo em Python. Considere Fs = 200 Hz.
#       (Obs.: o professor autorizou usar a Fs detectada pelo CSV)
#
...
# Frequencia de amostragem detectada pelo CSV (MIT-BIH = 360 Hz)
diffs = np.diff(t[:200])
...
# y3[n] = 2*(y1[n] + y2[n])   (combinacao final)
y3 = 2.0 * (y1 + y2)

# ──────────────────────────────────────────────
# 4. DETECCAO POR DUPLO LIMIAR
# ──────────────────────────────────────────────
lam1 = 0.5 * np.max(y3)   # limiar primario
lam2 = 0.1 * np.max(y3)   # limiar secundario
...
# ──────────────────────────────────────────────
# 5. GRAFICO: ECG + marcacoes QRS no mesmo grafico
# ──────────────────────────────────────────────
...
# ── Subplot 1: visao geral ECG + marcacoes QRS ──
...
# ── Subplot 2: zoom em torno do 3o QRS detectado ──
# Janela de +/- 0.3 s ao redor do QRS escolhido
idx_zoom = 2  # 3o QRS (evita bordas)
...
ax2.plot(t_z, ecg_z, color='steelblue', linewidth=1.2, label='ECG canal 1')
